...
table_status <- map(cons[-6],~print(sqlTables(.x, tableType = "TABLE"))) %>% 
  bind_rows() %>% 
...
calendar <- map(cons[-6],~sqlQuery(.x, "select * from calendar"))
has_calendar <- map(calendar,~class(.x) =="data.frame") %>% unlist()
calendar_table1 <- map2(calendar[has_calendar],df_db$dbname[-6][has_calendar],function(x,y){x %>% mutate(dbname = y)})
...
newroutes <- map(cons[-6],~sqlQuery(.x, "select * from newroutes"))
has_newroutes <- map(newroutes,~class(.x) =="data.frame") %>% unlist()
newroutes_table1 <- map2(newroutes[has_newroutes],df_db$dbname[-6][has_newroutes],function(x,y){x %>% mutate(dbname = y)})
newroutes_table1[[25]]$מסלול <- as.character(newroutes_table1[[25]]$מסלול)
...
testrips  <- map(cons[-6],~sqlQuery(.x, "select * from testrips"))
has_testrips<- map(testrips,~class(.x) =="data.frame") %>% unlist()
testrips_table1 <- map2(testrips[has_testrips],df_db$dbname[-6][has_testrips],function(x,y){x %>% mutate(dbname = y)})
...
trips  <- map(cons[-6],~sqlQuery(.x, "select * from trips"))
has_trips <- map(trips,~class(.x) =="data.frame") %>% unlist()
trips_table1 <- map2(trips[has_trips],df_db$dbname[-6][has_trips],function(x,y){x %>% mutate(dbname = y)})
trips_table1[[21]]$מסלול <- as.character(trips_table1[[21]]$מסלול)
...
trips_all <- map(cons[-6],~sqlQuery(.x, "select * from trips_all"))
has_trips_all <- map(trips_all,~class(.x) =="data.frame") %>% unlist()
trips_all_table1 <- map2(trips_all[has_trips_all],df_db$dbname[-6][has_trips_all],function(x,y){x %>% mutate(dbname = y)})

# map(1:27,~trips_all_table1[[.x]]$מסלול %>% class())
trips_all_table1[[25]]$מסלול <- as.character(trips_all_table1[[25]]$מסלול)
...
# have only 93 projects, out of 232 with data. concerning
bind_rows(dis_trips_not_matrices,dis_newroutes,dis_trips_all) %>% 
  select_if(not_all_na) %>% 
